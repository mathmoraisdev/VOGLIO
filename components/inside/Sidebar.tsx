...
const navigation = [
  { name: 'Dashboard', href: '/inside', icon: LayoutDashboard },
  { name: 'Projetos', href: '/inside/projects', icon: FolderKanban },
  { name: 'Perfil', href: '/inside/profile', icon: User },
]
    ...
    <div className="hidden lg:flex lg:flex-col lg:w-64 lg:fixed lg:inset-y-0 lg:border-r lg:border-gray-900 bg-black">
      <div className="flex flex-col flex-grow pt-8 pb-4 overflow-y-auto">
        <div className="flex items-center justify-center flex-shrink-0 px-6 mb-8">
          <Link href="/inside" className="flex items-center">
            <Image
              src="/logo.png"
              alt="Voglio"
              width={120}
        ...
        <nav className="flex-1 px-4 space-y-1">
          {navigation.map((item) => {
            const isActive = pathname === item.href || 
              (item.href !== '/inside' && pathname?.startsWith(item.href))
